...
async def run_pipeline(link_id: str) -> None:
    ...
    if not link.data or link.data["status"] != "extracted":
        logger.warning(f"Extraction did not complete for {link_id}, status={link.data and link.data.get('status')}")
        return
    ...
    try:
        ...
        logger.exception(f"Notes generation crashed for {link_id}")
    ...
    web_url = f"{settings.public_base_url.replace('-api.', '.').replace('/api', '')}"
    await _notify(
        f"📝 Notes ready: <b>{title[:60]}</b>\n"
        f"Open in app to read →"
    )
    logger.info(f"Pipeline complete (notes) for {link_id}")
